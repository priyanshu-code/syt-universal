// --- Solve Your Trip Service Tax Invoice Controller ---

// 1. Initial State Seeded with Sample PDF Data
let state = {
    ...
    destination: "Delhi",
    termsOfDelivery: "Facilitation fee strictly non-refundable. Re-bookings subject to supplier guidelines.",

    items: [
        {
            description: "SERVICE CHARGE (D)\nSANJEEV KUAMR CHANANA\nMUSKAN\nRANVEER",
            hsnSac: "998559",
            quantity: 3,
            rate: 169.49,
            per: "NOS",
    ...
    remarks: "DEL131431 x 3 SANJEEV KUAMR CHANANA x 3",
...
const pvGstColHeader1 = document.getElementById('pvGstColHeader1');
const pvGstColHeader2 = document.getElementById('pvGstColHeader2');
const pvGstTaxBreakdownTableBody = document.getElementById('pvGstTaxBreakdownTableBody');
const pvGstTaxBreakdownFoot = document.getElementById('pvGstTaxBreakdownFoot');
const pvTaxAmountInWords = document.getElementById('pvTaxAmountInWords');
...
function isSupplyIntrastate() {
    return state.guestState.toLowerCase().includes('delhi') || state.placeOfSupply?.toLowerCase().includes('delhi');
}
...
function runCalculations() {
    ...
    state.items.forEach(item => {
        let amt = Number(item.quantity || 0) * Number(item.rate || 0);
    ...
    const isDelhi = isSupplyIntrastate();

    if (isDelhi) {
        cgstAmount = Math.round(taxableTotal * 0.09 * 100) / 100;
        sgstAmount = Math.round(taxableTotal * 0.09 * 100) / 100;
        gstTotal = cgstAmount + sgstAmount;
    } else {
        igstAmount = Math.round(taxableTotal * 0.18 * 100) / 100;
        gstTotal = igstAmount;
    }
    ...
    state.taxAmountInWords = convertNumberToWords(gstTotal);
    pvAmountInWords.innerText = state.amountInWords;
    pvTaxAmountInWords.innerText = state.taxAmountInWords;

    // Render GST audit table
    pvGstTaxBreakdownTableBody.innerHTML = '';
    let sacGroups = {};
    state.items.forEach(item => {
        if (item.isTaxable) {
            let sac = item.hsnSac || 'N/A';
            let amt = Number(item.quantity || 0) * Number(item.rate || 0);
            if (!sacGroups[sac]) sacGroups[sac] = 0;
            sacGroups[sac] += amt;
        }
    });

    let totalGstTaxValue = 0;
    let totalCgstTaxValue = 0;
    let totalSgstTaxValue = 0;
    let totalIgstTaxValue = 0;

    const sacList = Object.keys(sacGroups);

    if (sacList.length === 0) {
        pvGstTaxBreakdownTableBody.innerHTML = `<tr><td colspan="7" class="text-center">No Taxable Items Found</td></tr>`;
    } else {
        sacList.forEach(sac => {
            let taxVal = sacGroups[sac];
            totalGstTaxValue += taxVal;

            let tr = document.createElement('tr');
            if (isDelhi) {
                let cgstVal = Math.round(taxVal * 0.09 * 100) / 100;
                let sgstVal = Math.round(taxVal * 0.09 * 100) / 100;
                let taxSum = cgstVal + sgstVal;
                totalCgstTaxValue += cgstVal;
                totalSgstTaxValue += sgstVal;

                tr.innerHTML = `
                    <td>${sac}</td>
                    <td class="text-right">₹ ${formatCurrency(taxVal)}</td>
                    <td class="text-center">9%</td>
                    <td class="text-right">₹ ${formatCurrency(cgstVal)}</td>
                    <td class="text-center">9%</td>
                    <td class="text-right">₹ ${formatCurrency(sgstVal)}</td>
                    <td class="text-right">₹ ${formatCurrency(taxSum)}</td>
                `;
            } else {
                let igstVal = Math.round(taxVal * 0.18 * 100) / 100;
                totalIgstTaxValue += igstVal;

                tr.innerHTML = `
                    <td>${sac}</td>
                    <td class="text-right">₹ ${formatCurrency(taxVal)}</td>
                    <td class="text-center">18%</td>
                    <td class="text-right">₹ ${formatCurrency(igstVal)}</td>
                    <td class="text-center">0%</td>
                    <td class="text-right">₹ 0.00</td>
                    <td class="text-right">₹ ${formatCurrency(igstVal)}</td>
                `;
            }
            pvGstTaxBreakdownTableBody.appendChild(tr);
        });
    }

    let totalGstTaxAmt = isDelhi ? (totalCgstTaxValue + totalSgstTaxValue) : totalIgstTaxValue;
    pvGstTaxBreakdownFoot.innerHTML = `
        <td style="font-weight: 800;">Total</td>
        <td class="text-right" style="font-weight: 800;">₹ ${formatCurrency(totalGstTaxValue)}</td>
        <td></td>
        <td class="text-right" style="font-weight: 800;">₹ ${formatCurrency(isDelhi ? totalCgstTaxValue : totalIgstTaxValue)}</td>
        <td></td>
        <td class="text-right" style="font-weight: 800;">₹ ${formatCurrency(isDelhi ? totalSgstTaxValue : 0)}</td>
        <td class="text-right" style="font-weight: 800;">₹ ${formatCurrency(totalGstTaxAmt)}</td>
    `;

    if (isDelhi) {
        pvGstColHeader1.innerText = "Central Tax";
        pvGstColHeader2.innerText = "State Tax";
    } else {
        pvGstColHeader1.innerText = "Integrated Tax";
        pvGstColHeader2.innerText = "N/A";
    }
}
    ...
    state.items.forEach((item, index) => {
        ...
        row.innerHTML = `
            ...
            <div style="display: flex; align-items: center; gap: 6px; margin-top: 2px;">
                <input type="checkbox" id="taxable-${index}" data-index="${index}" class="sidebar-item-taxable" ${item.isTaxable ? 'checked' : ''}>
                <label for="taxable-${index}" style="font-size: 11px; cursor: pointer; user-select: none;">Taxable (18% GST)</label>
            </div>
        `;
...
function renderPreviewTable() {
    ...
    let globalIndex = 1;
    let taxableItems = state.items.filter(item => item.isTaxable);
    let nonTaxableItems = state.items.filter(item => !item.isTaxable);
    ...
    // Subtotal tax injections
    let taxableValueSum = 0;
    taxableItems.forEach(item => {
        taxableValueSum += Number(item.quantity || 0) * Number(item.rate || 0);
    });
    ...
    if (taxableItems.length > 0) {
        ...
        spacerTr.className = 'tally-border-top';
        spacerTr.innerHTML = `<td colspan="7" style="height: 0; padding: 0 !important; border-left: none !important; border-right: none !important;"></td>`;
        pvInvoiceTableBody.appendChild(spacerTr);

        if (isDelhi) {
            let cgstVal = Math.round(taxableValueSum * 0.09 * 100) / 100;
            let sgstVal = Math.round(taxableValueSum * 0.09 * 100) / 100;

            let trCgst = document.createElement('tr');
            trCgst.innerHTML = `
                <td></td>
                <td style="font-weight: 700;">CGST @ 9%</td>
                <td></td>
                <td></td>
                <td></td>
                <td class="text-center">9%</td>
                <td class="text-right" style="font-weight: 700;">${formatCurrency(cgstVal)}</td>
            `;
            pvInvoiceTableBody.appendChild(trCgst);

            let trSgst = document.createElement('tr');
            trSgst.innerHTML = `
                <td></td>
                <td style="font-weight: 700;">SGST @ 9%</td>
                <td></td>
                <td></td>
                <td></td>
                <td class="text-center">9%</td>
                <td class="text-right" style="font-weight: 700;">${formatCurrency(sgstVal)}</td>
            `;
            pvInvoiceTableBody.appendChild(trSgst);
        } else {
            let igstVal = Math.round(taxableValueSum * 0.18 * 100) / 100;
            let trIgst = document.createElement('tr');
            trIgst.innerHTML = `
                <td></td>
                <td style="font-weight: 700;">IGST @ 18%</td>
                <td></td>
                <td></td>
                <td></td>
                <td class="text-center">18%</td>
                <td class="text-right" style="font-weight: 700;">${formatCurrency(igstVal)}</td>
            `;
            pvInvoiceTableBody.appendChild(trIgst);
        }
    }
...
itemRowsContainer.addEventListener('change', (e) => {
    if (e.target.classList.contains('sidebar-item-taxable')) {
        const idx = parseInt(e.target.dataset.index);
        state.items[idx].isTaxable = e.target.checked;
        updateUI();
    }
});
...
btnAddInvoiceItem.addEventListener('click', () => {
    state.items.push({
        description: "New Service Description Line",
        hsnSac: "998559",
        quantity: 1,
        rate: 0,
        per: "NOS",
...
btnExport.addEventListener('click', () => {
    ...
    downloadAnchor.setAttribute("download", `invoice_service_${safeInvoiceNo}.json`);
...
window.addEventListener('DOMContentLoaded', () => {
    const saved = localStorage.getItem('syt_service_invoice_state');
    if (saved) {
        try {
