...
let state = {
    ...
    guestName: "Kulwinder Singh",
    guestAddress: "Flat 402, Sea Breeze, Bandra West, Mumbai, MH - 400050",
    guestState: "Maharashtra",
    guestStateCode: "27",

    invoiceNo: "SYT-26-27/0001",
    ...
    destination: "Mumbai",
    termsOfDelivery: "Standard facilitation rates. Subject to customer agreements.",

    remarks: "DEL131431 x 3 Booking processing charges.",
    ...
    showStamp: true,
    signatureType: "image", // 'image' or 'digital'
    ...
    items: [
        {
            description: "Facilitation Fee for Singapore Visa Processing",
            hsnSac: "9985",
            quantity: 3,
            rate: 1500,
            per: "NOS",
    ...
    roundOff: 0,
    totalQty: 3,
    amountInWords: "INR Four Thousand Five Hundred Rupees Only."
};
...
document.addEventListener('input', (e) => {
    ...
    if (e.target.hasAttribute('contenteditable')) {
        ...
        if (id === 'pvStampCompany') {
            state.companyName = val.toUpperCase();
            if (document.activeElement !== elCompanyName) elCompanyName.value = val;
            const el = document.getElementById('pvStampDirector');
            if (el) el.innerText = 'DIRECTOR - ' + state.directorName.toUpperCase();
        }
...
btnExport.addEventListener('click', () => {
    ...
    downloadAnchor.setAttribute("download", `invoice_nongst_${safeInvoiceNo}.json`);
...
window.addEventListener('DOMContentLoaded', () => {
    const saved = localStorage.getItem('syt_nongst_invoice_state');
    if (saved) {
        try {
